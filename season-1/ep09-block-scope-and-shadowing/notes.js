...
// - Shadowing `var` vs shadowing `let`/`const` behaves differently: shadowing a `var` inside a block actually **modifies the outer `var`** too (since `var` isn't block-scoped), whereas shadowing `let`/`const` keeps the outer variable untouched.
...
(function () {
  ...
  let c = 10;
  {
    // var c = 20; // SyntaxError: Identifier 'c' has already been declared (illegal shadowing) — commented out so file stays runnable
  }
})();
//
//
