...
//
// --- Key Concepts ---
// - **`undefined`**: Memory has been allocated for the variable (during the memory creation phase), but no value has been assigned to it yet. It's a special placeholder value, and also a type in JS.
// - **Not defined**: The variable doesn't even exist in memory — it was never declared. Accessing it throws a `ReferenceError`.
// - Assigning `undefined` to a variable explicitly (`var a = undefined`) is technically allowed but considered **bad practice**, since JS itself uses `undefined` as its own placeholder and it becomes hard to distinguish "not yet assigned" from "explicitly set to undefined."
//
//
// --- Example ---
