...
- `let` and `const` declarations **are** hoisted (contrary to popular belief), but they are placed in a separate memory space than `var`, and are **not initialized** with `undefined`. They remain uninitialized until the line of code that declares them is executed.
